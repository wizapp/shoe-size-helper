class ShoeSizeManager {
  ...
  buildShoeSizePerType(measurmentType) {
    ...
    let res = _finalShoeSizes.sort((a, b) => {
      if (a.foot < b.foot) {
        return -1;
      }
      if (a.foot > b.foot) {
  ...
  searchFootSize(shoeSize, zone, type) {
    ...
    const zoneFootSizes = shoeSizes.filter(_shoeSize => _shoeSize.sizes[zone]).map(_shoeSize => {
      return { foot: _shoeSize.foot, size: _shoeSize.sizes[zone] };
    });
    ...
    const keptShoeSizes = zoneFootSizes.filter(_zoneFootSize => {
      return Math.abs(_zoneFootSize.size - shoeSize) < shoeSizeTolerance;
    });

    // Find nearest footSize for given shoeSize
    // => Sort by difference !
    keptShoeSizes.sort((a, b) => {
      return Math.abs(shoeSize - a.size) < Math.abs(shoeSize - b.size) ? -1 : 1;
    });
  ...
  findShoeSize(footSize, zone, type) {
    ...
    // Filter shoe size for which they are valid for zone and size is not too far !
    const keptShoeSizes = shoeSizes.filter(shoeSize => {
      return (
        shoeSize.sizes[zone] != undefined && Math.abs(footSize - shoeSize.foot) < footSizeTolerance
      );
    });
    ...
    // => Sort by difference !
    keptShoeSizes.sort((a, b) => {
      return Math.abs(footSize - a.foot) < Math.abs(footSize - b.foot) ? -1 : 1;
    });
    const foundShoeSize = keptShoeSizes[0];
    if (!foundShoeSize) {
      return;
